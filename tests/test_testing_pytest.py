...
def test_junitxml(
    ...
    testcases = sel.xpath(
        "//testcase[@classname='fixtures.tests.test_testing_pytest.MyItemPage.test-1.output.json' and @name='foo']"
    )
...
def test_junitxml_expected_exception(
    ...
    sel = Selector(report_path.read_text(encoding="utf-8"))
    testcases = sel.xpath(
        "//testcase[@classname='fixtures.tests.test_testing_pytest.RetryItemPage.test-1.exception.json' and @name='TO_ITEM_RAISES']"
    )
    assert len(testcases) == 1
    testcase = testcases[0]
    expected_node = testcase.xpath(
